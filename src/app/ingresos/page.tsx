...
interface AppointmentWithoutCustomer {
  id: string;
  date: string;
  duration: number;
  status: string;
...
export default function IngresosPage() {
  ...
  const [viewMode, setViewMode] = useState<
    "overview" | "history" | "clients" | "projections"
  >("overview");
  ...
  const fetchIncomeData = async () => {
    try {
      ...
      const thisMonthCompleted = (thisMonthData.appointments || []).filter(
        (apt: AppointmentData) => apt.status === "COMPLETED"
      );
      const lastMonthCompleted = (lastMonthData.appointments || []).filter(
        (apt: AppointmentData) => apt.status === "COMPLETED"
      );
      const thisYearCompleted = (thisYearData.appointments || []).filter(
        (apt: AppointmentData) => apt.status === "COMPLETED"
      );
      ...
      const pendingAppointments = (thisMonthData.appointments || []).filter(
        (apt: AppointmentData) =>
  ...
  const fetchAppointments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const appointmentsWithIncome = (data.appointments || []).map(
          (apt: AppointmentData) => ({
            ...apt,
            income:
  ...
  const fetchClientMetrics = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const metricsPromises = (data.customers || []).map(
          async (customer: CustomerBasic) => {
            try {
              ...
              if (customerRes.ok) {
                ...
                ).filter((apt: AppointmentData) => apt.status === "COMPLETED");
                const totalIncome =
                  completedAppointments.length * APPOINTMENT_PRICE;

                const appointmentDates = (customerData.appointments || []).map(
                  (apt: AppointmentData) => new Date(apt.date)
                );
                const firstAppointment = Math.min(
                  ...appointmentDates.map((d: Date) => d.getTime())
                );
                const monthsSinceFirst =
                  (Date.now() - firstAppointment) / (1000 * 60 * 60 * 24 * 30);
                const averagePerMonth =
                  Math.max(monthsSinceFirst, 1) > 0
                    ? customerData.appointments.length /
                      Math.max(monthsSinceFirst, 1)
                    : 0;
                ...
                const recent = (customerData.appointments || []).filter(
                  (apt: AppointmentData) => new Date(apt.date) >= threeMonthsAgo
                ).length;
                const previous = (customerData.appointments || []).filter(
                  (apt: AppointmentData) => {
                ...
                return {
                  ...
                  averagePerAppointment:
                    totalIncome / Math.max(completedAppointments.length, 1),
                  lastAppointment:
                    customerData.appointments?.length > 0
                      ? customerData.appointments.sort(
                          (a: AppointmentData, b: AppointmentData) =>
    ...
    try {
      ...
      if (response.ok) {
        ...
        const customer = data.customers?.find(
          (c: CustomerBasic) => c.name === customerName
        );
        if (customer) {
  ...
  const calculateCustomerIncome = (
    appointments: AppointmentWithoutCustomer[]
  ) => {
    return appointments
              ...
              {incomeData && (
                <div
                  className={`flex items-center mt-1 ${
                    incomeData.growth >= 0 ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {incomeData.growth >= 0 ? (
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {incomeData
                  ? formatCurrency(
                      (incomeData.pendingAppointments +
                        incomeData.futureAppointments) *
                        APPOINTMENT_PRICE
                    )
                  : "..."}
              </p>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      ...
                      {client.averageAppointmentsPerMonth} promedio/mes
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="flex items-center mt-1">
                    {client.growthTrend === "up" && (
                      <ArrowUpIcon className="h-4 w-4 text-green-500" />
                    )}
                    {client.growthTrend === "down" && (
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <select
            value={sortBy}
            onChange={(e) =>
              setSortBy(e.target.value as "date" | "income" | "client")
            }
            ...
            {getSortedAppointments().map((appointment) => (
              <tr key={appointment.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    onClick={() =>
                      openCustomerDetailsByName(appointment.customer.name)
                    }
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {clientMetrics.map((client) => (
              <tr key={client.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">
                    {client.averageAppointmentsPerMonth}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">
        ...
        <div className="flex space-x-1 mb-6">
          {[
            ...
            <button
              ...
              onClick={() =>
                setViewMode(
                  key as "overview" | "history" | "clients" | "projections"
                )
              }
